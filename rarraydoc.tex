...
\date{December, 2023 (version 2.7.0, unreleased)\vspace{-7mm}}
...
\subsection{Assigning values to the elements of an rarray}

In addition to the explicit assignment of each element as in the
example above, there are a few
ways to assign values to the elements of an rarray as a whole.  One is
the comma separate form, as follows
\vspace{-9pt}
\begin{framed}\vspace{-18pt}%
\begin{verbatim}
...
Another way to set the values of the element is to use nested
initializer lists, from which the shape can be deduced.  The library
offers are two method to do this:
\begin{enumerate}
  \item The \texttt{fill} method, which changes the values of an
    already existing rarray.
  \item The \texttt{form} method, which discard any existing array
    and reconstructs a new rarray whose shape is based on the
    parameters passed to the method.
\end{enumerate}

\subsubsection{Filling an rarray}

The \texttt{fill} family of methods assigns values to an already
constructed rarray within its existing shape.  Filling with a uniform
value is the simplest case:
\vspace{-9pt}
\begin{framed}\vspace{-18pt}%
\begin{verbatim}
  rarray<double,2> matrix(3,3);
...
\vspace{-12pt}
\end{framed}\vspace{-8pt}\noindent
In addition, there is a \texttt{fill} method that takes nested
initializer lists, like so:
\vspace{-9pt}
\begin{framed}\vspace{-18pt}%
\begin{verbatim}
  rarray<double,2> matrix(3,3);
...
a \texttt{ra::MISSING} argument can also be established by
passing \texttt{ra::MISSING::DEFAULT} as the second argument to the
\texttt{fill} method.

\newpage
\subsubsection{Forming an rarray}

The \texttt{fill} methods require that the rarray already has a
shape, and its storage is reused.  In contrast, the \texttt{form}
method create and allocate this storage.  Any existing storage and
shape is discarded as if the array went out of scope.
...
\vspace{-12pt}
\end{framed}\vspace{-8pt}\noindent
Because this rarray's rank is 2, the first two arguments specify the
rarray's new shape, where as the last argument (here, \texttt{1.0}) is what the
matrix will be filled with.

Another form takes nested
initializer lists, like so:
\vspace{-9pt}
\begin{framed}\vspace{-18pt}%
\begin{verbatim}
...
\vspace{-12pt}
\end{framed}\vspace{-8pt}\noindent
The size of the matrix is determined from the number of elements in
the initializer lists.
Similar to the situation with \texttt{fill},
if there are missing elements, those spots will be assigned the
default value corresponding to the type (which amounts to 0 for
numerical types). Thus, one can for instance create a 3x3 matrix of zeros with
\vspace{-9pt}
\begin{framed}\vspace{-18pt}%
\begin{verbatim}
  rarray<double,2> matrix;
...
\vspace{-12pt}
\end{framed}\vspace{-8pt}\noindent
As with \texttt{fill}, one can specify other
...
\vspace{-12pt}
\end{framed}\vspace{-8pt}\noindent
creates an array with 1s in the first column, 2s in the second column
and 3s in the third column.  While
\vspace{-9pt}
\begin{framed}\vspace{-18pt}%
\begin{verbatim}
  rarray<double,2> matrix;
...
When using \texttt{ra::MISSING::SKIP} instead of \texttt{ra::MISSING::REPEAT}, the
missing elements are not initialized.

As will be explained in the next two sections, assignment of rarrays
as a whole
(e.g. \texttt{a=b} where \texttt{a} and \texttt{b} are both rarray objects of
the same rank) works as if they are reference counted pointer.   

\subsection{rarray assignment and function arguments}

In C++, when we assign a variable of a built-in type to a new variable,
...
The reshape method will never allocate any
...
public:    
 ...
 rarray<T,R-1> at(size_type i);      // retrieve the ith 'row' 
 rarray<T,R> slice(size_type begin, size_type end);  // get subarray
};
}
...
This method fills the array with a uniform \texttt{value}.

\noindent\textbf{Definition:} \texttt{void  fill(std::initializer\_list<....<T>>> list, ra::MISSING policy);}

This method fills the array according to the nested
initializer. Handles missing elements in the initializer according to
policy.

\noindent\textbf{Definition:} \texttt{void form(size\_type n0, .... const T\& value);}

This method recreates (``forms'') the array with given dimensions, filled with a uniform \texttt{value}.

\noindent\textbf{Definition:} \texttt{void  form(std::initializer\_list<....<T>>> list, ra::MISSING \texttt{policy});}

This method recreates the array with dimensions that fit the nested
initializer \texttt{list}. Handles missing elements in the initializer
according to \texttt{policy}.
...
\noindent\textbf{Definition:} \texttt{rarray<T,R-1> at(size\_t i);}

Retrieves the ith 'row', with bounds checking. The returned array is
one rank lower than the original and contains a (counted) reference to
just that row of the original.
