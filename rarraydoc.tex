...
reshape to do its work by giving an additional (optional) argument
`\texttt{RA\_FORCE}' as the last argument to '\texttt{reshape}'. So the following would work
\vspace{-9pt}
\begin{framed}\vspace{-18pt}%
\begin{verbatim}
void dump(rarray<double,3> & r) {
   r.reshape(r.data(), 1, 1, r.size(), RA_FORCE);
   for (int i=0;i<r.size();i++)
       std::cout << rflat[i] << ' ';
  std::cout << std::endl;
}
