...
\date{January 12, 2014\vspace{-7mm}}
...
\item Checks index bounds if the preprocessor
  constant {\tt AR\_BOUNDSCHECK} is defined. 
...
If the preprocessor constant \texttt{{\tt AR\_BOUNDSCHECK}} is defined, an
...
\end{itemize}
\texttt{{\tt AR\_BOUNDSCHECK}} can be defined by
adding the {\tt -DAR\_BOUNDSCHECK} argument to the compilation command, or
by \texttt{{\tt\#define AR\_BOUNDSCHECK}} before
the \texttt{{\tt\#include "rarray.h"}} in the source.
%\\
%Regardless of \texttt{{\tt AR\_BOUNDSCHECK}}, an exception is raised if the %memory allocation for rarray fails.
...
So in that sense, no there is no performance overhead.  However, with \texttt{{\tt AR\_BOUNDSCHECK}} defined, one incurs a compiler and machine dependent performance hit.
...
To get full speedup for these less-optimizing compilers, you can define the preprocessor constant \texttt{AR\_SKIPINTERMEDIATE} (usually with a \texttt{-DAR\_SKIPINTERMEDIATE} compiler option).  Intermediate expression in a multi-bracketed expression are then replaced by bare pointers-to-pointers.

Note that when \texttt{AR\_BOUNDSCHECK} is set, it will switch off \texttt{AR\_SKIPINTERMEDIATE}.
